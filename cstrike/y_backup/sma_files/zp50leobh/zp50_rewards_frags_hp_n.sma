...
new cvar_infection_health_bonus, cvar_kill_zm_health_bonus
new cvar_human_last_health_bonus

public plugin_init()
{
	register_plugin("[ZP] Rewards: Frags & HP", ZP_VERSION_STRING, "Leo_[BH] & ZP Dev Team")
	...
	cvar_kill_zm_health_bonus = register_cvar("zp_kill_zombie_health_bonus", "20")
...
public hook_death()
{
	...
	if(!zp_core_is_zombie(killer))
	{
		if(is_user_alive(killer))
			set_user_health(killer, get_user_health(killer) + get_pcvar_num(cvar_kill_zm_health_bonus))
	}
}
