...
new cvar_respawns_count

new has_ReSpawns[33] = 0

public plugin_init()
{
	register_plugin("[ZP] Deathmatch", ZP_VERSION_STRING, "Leo_[BH] & ZP Dev Team")
	...
	cvar_respawns_count = register_cvar("zp_respawns_count", "2")
...
public respawn_player_task(taskid)
{
	...
	if(has_ReSpawns[ID_RESPAWN]) // Проверка на количество возрождений:
	{
		if(has_ReSpawns[ID_RESPAWN] == 1)
		{
			zp_colored_print(ID_RESPAWN, "^3%L", ID_RESPAWN, "LAST_SPAWN")
		}
	}
	else
	{
		zp_colored_print(ID_RESPAWN, "^3%L", ID_RESPAWN, "MAX_SPAWNS")
		return
	}
...
public zp_fw_gamemodes_start()
{
	...
	for (id = 1; id <= 32; id++)
	{
		has_ReSpawns[id] = get_pcvar_num(cvar_respawns_count)
	}
}
