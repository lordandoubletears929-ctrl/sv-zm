...
#define LM_COST 0 // ZP50 menu cost is 0 because we charge ammo packs manually below.

// Custom settings added for ZP 5.0 version
#define LM_BUY_LIMIT_PER_ROUND 2
#define LM_COST_FIRST_BUY 15
#define LM_COST_SECOND_BUY 20
#define LM_PLACE_DELAY 4.0
#define LM_PLACE_DELAY_SOUND "lasermine_perfect/place.wav"
...
#if defined EXTRA_ITEM_VERSION
new g_itemid, g_lm_ammo[33], cvar_start_ammo, g_lm_bought[33], g_lm_planting[33]
#endif
...
public plugin_init() {
	...
	// Command Register
	register_clcmd("+setlaser", "Create_Lasermine_Delay");
	...
	register_event("HLTV", "LM_RoundStart", "a", "1=0", "2=0");
...
public plugin_natives() {
	...
	register_native("zp_lasermine_get_cost", "native_lasermine_get_cost", 1)
...
// Returns current Lasermine cost for the custom hub/menu.
// 1st buy = 15 AP, 2nd buy = 20 AP.
public native_lasermine_get_cost(id)
{
	return get_lm_buy_cost(id);
}
...
public plugin_precache() {
	precache_generic("sound/lasermine_perfect/place.mp3");
...
// Create Lasermine with delay
public Create_Lasermine_Delay(id) {
	if(!is_user_alive(id))
		return PLUGIN_HANDLED;
	...
	// Sound starts immediately when +setlaser is pressed, before the delay finishes.
	client_cmd(id, "mp3 play sound/lasermine_perfect/place.mp3");

	set_task(LM_PLACE_DELAY, "Create_Lasermine_Task", id + TASK_PLACE_DELAY);

	return PLUGIN_HANDLED;
}

public Create_Lasermine_Task(taskid) {
	new id = taskid - TASK_PLACE_DELAY;
	g_lm_planting[id] = 0;
...
public Create_Lasermine(id) {
	...
	static Float:vNewOrigin[3], Float:vNormal[3], Float:vSurfaceNormal[3], Float:vTraceDirection[3], Float:vTraceEnd[3];
	pev(id, pev_origin, vOrigin);
	vOrigin[2] += 15
	velocity_by_aim(id, 200, vTraceDirection);
	...
	if (fFraction < 1.0) {
		...
		get_tr2(0, TR_vecPlaneNormal, vSurfaceNormal);
	}

	// Save the original surface normal for angle calculation and laser direction.
	vNormal[0] = vSurfaceNormal[0];
	vNormal[1] = vSurfaceNormal[1];
	vNormal[2] = vSurfaceNormal[2];

	static Float:vPlaceOffset[3];
	xs_vec_mul_scalar(vSurfaceNormal, 2.0, vPlaceOffset);
	xs_vec_add(vTraceEnd, vPlaceOffset, vNewOrigin);

	engfunc(EngFunc_SetSize, i_Ent, Float:{ -0.3, -0.3, -0.3 }, Float:{ 0.3, 0.3, 0.3 });
	engfunc(EngFunc_SetOrigin, i_Ent, vNewOrigin);

	// -- Rotate tripmine. Floors/ceilings use player's yaw so it no longer always points sideways.
	Set_Lasermine_Angles(i_Ent, id, vNormal);
...
public Lasermine_TakeDamagePre(victim, inflictor, attacker, Float:f_Damage, bit_Damage) 
{
	...
	// Block damage from world/entities/invalid attackers
	if(!is_user_connected(attacker))
		return HAM_SUPERCEDE;

	// Humans cannot damage lasermines
	if(!zp_core_is_zombie(attacker))
		return HAM_SUPERCEDE;

	// Zombies can damage lasermines
	return HAM_IGNORED;
} 
...
public Lasermine_Hud(iTaskIndex) {
	...
	if(!is_entity_pdata_valid(iEntity))
		return;

	pev(iEntity, pev_classname, cClassname, charsmax(cClassname));

	if(is_user_alive(iPlayer) && equal(cClassname, Lasermine_Classname) && get_pcvar_num(cvar_lm[8])) {
		...
		// First line: only owner nickname. Second line: health stays the same idea.
		show_hudmessage(iPlayer, "%s^nHealth: %d", name, pev(iEntity, pev_health));
	}
} 
...
public client_disconnect(id) {
	...
	remove_task(id + TASK_PLACE_DELAY);
	g_lm_planting[id] = 0;
	remove_task(id + TASK_HUD)
}

// New Round
public LM_RoundStart() {
	// Remove all lasermines only when a new round starts.
...
public Spawn_Event(id) {
	// Do not remove lasermines on player spawn.
	// They are removed globally at the start of a new round.
...
public DeathEvent() {
	...
	id = read_data(2)
	if(is_user_connected(id)) {
		// Do not remove lasermines when the owner dies.
		// They stay until the next round starts.
		g_lasermine_imune[id] = false
	}
	return PLUGIN_CONTINUE
}
...
public zp_fw_gamemodes_end() {
	...
	// Do not remove lasermines here.
	// They are removed only when the next round starts.
}
...
public lm_configs_menu_handler(id, key) { 
	...
	switch(key+1) {
		...
		case 8: Create_Lasermine_Delay(id), lm_configs_menu(id); // Plant Lasermine
...
public zp_fw_items_select_pre(id, itemid)
{
	...
	static text[48], cost;
	cost = get_lm_buy_cost(id);
	formatex(text, charsmax(text), "[%d/%d] [%d AP]", g_lm_bought[id], LM_BUY_LIMIT_PER_ROUND, cost);
	zp_items_menu_text_add(text);

	if(g_lm_bought[id] >= LM_BUY_LIMIT_PER_ROUND)
...
public zp_fw_items_select_post(id, itemid) 
{
	if(g_itemid == itemid) {
		if(g_lm_bought[id] >= LM_BUY_LIMIT_PER_ROUND) {
			client_printcolor(id, "%L Maximo %d compras de Lasermine por ronda.", id, "CHATTAG", LM_BUY_LIMIT_PER_ROUND)
			return;
		}
		...
		if(zp_get_user_ammo_packs(id) < cost) {
			client_printcolor(id, "%L No tienes suficientes ammo packs. Necesitas %d.", id, "CHATTAG", cost)
			return;
		}
...
#if defined EXTRA_ITEM_VERSION
stock get_lm_buy_cost(id)
{
	if(g_lm_bought[id] <= 0)
		return LM_COST_FIRST_BUY;

	return LM_COST_SECOND_BUY;
}
#endif

stock Set_Lasermine_Angles(i_Ent, id, const Float:vNormal[3])
{
	static Float:vEntAngles[3], Float:vPlayerAngles[3];

	vector_to_angle(vNormal, vEntAngles);

	// Horizontal surfaces (floor/ceiling) have no natural yaw.
	// Use the player's yaw so the mine orientation follows where the player is facing.
	if(floatabs(vNormal[2]) > 0.70)
	{
		pev(id, pev_angles, vPlayerAngles);
		vEntAngles[1] = vPlayerAngles[1];
		vEntAngles[2] = 0.0;
	}
